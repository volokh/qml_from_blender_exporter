...
def image_from_normal_input(normal_input):
    ...
    if not nm:
        return None, None
    strength = nm.inputs["Strength"].default_value if "Strength" in nm.inputs else getattr(nm, "strength", 1.0)
...
def principled_bsdf_to_quick3d(mat, img_dir, exported_images, indent=0):
    ...
    def tex_source_from_image(img):
        ...
        return rel_#img.filepath.replace("\\", "/") if img.filepath else img.name
    ...
    emission_color = val("Emission Color", val("Emission", (0.0, 0.0, 0.0, 1.0)))
    ...
    clearcoat_rough = float(val("Coat Roughness", val("Clearcoat Roughness", 0.03)))
    ...
    emissive_img = image_from_socket_or_normal_chain(inp("Emission Color")) or image_from_socket_or_normal_chain(inp("Emission"))
    opacity_img = image_from_socket_or_normal_chain(inp("Alpha"))
    transmission_img = image_from_socket_or_normal_chain(inp("Transmission Weight")) or image_from_socket_or_normal_chain(inp("Transmission"))
    normal_img, normal_strength = image_from_normal_input(inp("Normal"))

    clearcoat_img = image_from_socket_or_normal_chain(inp("Coat Weight")) or image_from_socket_or_normal_chain(inp("Clearcoat"))
    clearcoat_rough_img = image_from_socket_or_normal_chain(inp("Coat Roughness")) or image_from_socket_or_normal_chain(inp("Clearcoat Roughness"))
    clearcoat_normal_img, clearcoat_normal_strength = image_from_normal_input(inp("Coat Normal"))
    ...
    if opacity_img:
        lines.append(f'{ind1}opacityMap: Texture {{ source: "{tex_source_from_image(opacity_img)}" }}')
    ...
    if transmission > 0.0:
        ...
        if transmission_img:
            lines.append(f'{ind1}transmissionMap: Texture {{ source: "{tex_source_from_image(transmission_img)}" }}')
    ...
    if clearcoat > 0.0:
        ...
        if clearcoat_img:
            lines.append(f'{ind1}clearcoatMap: Texture {{ source: "{tex_source_from_image(clearcoat_img)}" }}')
        if clearcoat_rough_img:
            lines.append(f'{ind1}clearcoatRoughnessMap: Texture {{ source: "{tex_source_from_image(clearcoat_rough_img)}" }}')
        lines.append(f"{ind1}clearcoatRoughnessAmount: {clamp01(clearcoat_rough):.6f}")
        lines.append(f"{ind1}clearcoatRoughnessChannel: PrincipledMaterial.R")
        if clearcoat_normal_img:
            lines.append(f'{ind1}clearcoatNormalMap: Texture {{ source: "{tex_source_from_image(clearcoat_normal_img)}" }}')
            lines.append(f"{ind1}clearcoatNormalStrength: {clearcoat_normal_strength}")
        else:
            lines.append(f"{ind1}clearcoatNormalStrength: {clearcoat_normal_strength}")
    else:
        lines.append(f"{ind1}clearcoatAmount: 0.0")
        lines.append(f"{ind1}clearcoatRoughnessAmount: 0.03")
        lines.append(f"{ind1}clearcoatFresnelBias: 0.0")
        lines.append(f"{ind1}clearcoatFresnelPower: 5.0")
        lines.append(f"{ind1}clearcoatFresnelScale: 1.0")
        lines.append(f"{ind1}clearcoatFresnelScaleBiasEnabled: false")
